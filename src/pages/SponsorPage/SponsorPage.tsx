...
function PostCard({ postIndex, onDetailsClick }: { postIndex: number; onDetailsClick: () => void }) {
  const [liked, setLiked] = useState(false);
  const [likeCount, setLikeCount] = useState(211);

  return (
    <div className="bg-[#232227] rounded-[18px] py-5 px-4 flex flex-col gap-5">
      ...
      <div className="flex flex-col gap-3">
        <p className="font-inter text-[16px] text-white leading-[100%]">
          Welcome to Our Platform
        </p>
        <p className="font-inter text-[16px] text-white leading-[100%]">
          We create modern digital solutions that help users navigate easily, make informed decisions, and enjoy a convenient online experience.
        </p>
        <div className="flex flex-col gap-1">
          <p className="font-inter text-[16px] text-white leading-[100%]">
            Why Choose Us
          </p>
          <ul className="list-disc pl-5 font-inter text-[16px] text-white leading-[100%]">
            <li>Clear and user-friendly interface</li>
            <li>Access from any device</li>
            <li>Fast registration and quick start</li>
            <li>User support available 24/7</li>
          </ul>
        </div>
      </div>
      ...
      <div className="flex flex-col gap-3">
        <div className="flex gap-[11px]">
          <button className="flex items-center gap-1.5 bg-[#37afe3] rounded-[8px] px-2 py-2 active:scale-95 transition-transform duration-100 min-w-0 flex-1">
            ...
            <span className="font-inter font-bold text-[10px] text-white text-center leading-[150%]">Telegram</span>
          </button>
          <button className="flex items-center gap-1.5 bg-[#26d367] rounded-[8px] px-2 py-2 active:scale-95 transition-transform duration-100 min-w-0 flex-1">
            <svg className="shrink-0" width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
              <mask id={`mask_wa_${postIndex}`} maskUnits="userSpaceOnUse" x="0" y="0" width="20" height="20">
                <path d="M0 0H19.3257V19.3257H0V0Z" fill="white" />
              </mask>
              <g mask={`url(#mask_wa_${postIndex})`}>
            ...
            <span className="font-inter font-bold text-[10px] text-white text-center leading-[150%]">WhatsApp</span>
          </button>
          <button className="flex items-center gap-1.5 bg-[#f60d8a] rounded-[8px] px-2 py-2 active:scale-95 transition-transform duration-100 min-w-0 flex-1">
            <svg className="shrink-0" width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            <span className="font-inter font-bold text-[10px] text-white text-center leading-[150%]">Instagram</span>
          </button>
        </div>
      ...
      <main className="flex-1 flex flex-col px-4 pt-4 pb-[calc(90px+env(safe-area-inset-bottom,0px))] relative z-[1] overflow-y-auto">
        ...
        <div className="relative flex items-center justify-between mb-4 mt-5 shrink-0">
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center gap-2">
            <span className="font-inter font-bold text-[18px] text-white leading-[83%]">
              Название канала
            </span>
            <span className="font-inter text-[12px] text-[#a6a6a6] leading-[125%] font-bold">
              10422 подписчика
            </span>
          </div>
          <button
        ...
        <div className="flex flex-col gap-4">
          <PostCard postIndex={0} onDetailsClick={() => setShowDetails(true)} />
          <PostCard postIndex={1} onDetailsClick={() => setShowDetails(true)} />
          <PostCard postIndex={2} onDetailsClick={() => setShowDetails(true)} />
        </div>
