    ...
    <div className="h-dvh flex flex-col relative bg-gradient-to-b from-black to-[#b42115] overflow-hidden">
      <main className="flex-1 flex flex-col items-center justify-center px-4 pb-[calc(90px+env(safe-area-inset-bottom,0px))] relative z-[1] gap-6">
        ...
        <img
          src={coinImage}
          alt="Euro coin"
          className="w-[clamp(160px,50vw,240px)] h-auto select-none drop-shadow-[0_8px_24px_rgba(0,0,0,0.5)]"
          draggable={false}
        />

        <span className="font-inter font-black text-[40px] leading-[38%] text-center text-white">
          {balance.toFixed(2)} EUR
        </span>

        <button
          className="flex items-center gap-[24px] bg-black rounded-[11px] py-4 mt-[30px] px-8 active:scale-[0.97] transition-transform duration-100"
        ...
        >
          ...
          <span className="font-inter font-medium text-[24px] leading-[63%] text-white">
            Вывод средств
          </span>
        </button>
      </main>
